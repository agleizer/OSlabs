/* SISTEMAS OPERACIONAIS - PROJETO 01
 * Universidade Presbiteriana Mackenzie
 * Ciência da Computação - Turma 04P11
 * 
...
/* OBSERVAÇÃO
 * os ajustes para funcionamento correto do programa foram realizados nas linhas: 46, 49, 65, 72
 */
...
// versão original
void depositANTIGO(BankAccount *account, int amount) {
    account->balance += amount;
    printf("Depositado: %d, Saldo Atual: %d\n", amount, account->balance);
}

// versão consertada, com uso correto do mutex
void deposit(BankAccount *account, int amount) {
    pthread_mutex_lock(&account->mutex);  // lock o mutex antes de modificar a variável balance
    account->balance += amount;
    printf("Depositado: %d, Saldo Atual: %d\n", amount, account->balance);
    pthread_mutex_unlock(&account->mutex);  // destranca o mutex após o fim da função
}

// Função para sacar uma quantia da conta
// versão original
void withdrawANTIGO(BankAccount *account, int amount) {
